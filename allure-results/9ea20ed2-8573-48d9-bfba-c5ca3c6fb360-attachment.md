...
- Name: web\product-search-flow.spec.ts >> Product Search Flow @master @sanity @regression @web
- Location: tests\web\product-search-flow.spec.ts:16:5
...
```ts
  ...
  17 |         this.myAccountDropdown = page.locator('a[title="My Account"]');
  18 |         this.registerLink = page.locator('ul.dropdown-menu li a', { hasText: 'Register' });
  19 |         this.loginLink = page.locator('ul.dropdown-menu li a', { hasText: 'Login' });
  20 |         this.searchInput = page.locator('input[name="search"]');
  21 |         this.searchButton = page.locator('button:has-text("")');
  22 |     }
  23 | 
  24 |     /**
  25 |      * Navigates to the home page
  26 |      */
  27 |     async goto(): Promise<void> {
  28 |         await this.page.goto(process.env.WEB_APP_URL || 'http://localhost/opencart/upload/');
  29 |     }
  30 | 
  31 |     /**
  32 |      * Opens the My Account dropdown menu
  33 |      */
  34 |     async openMyAccountDropdown(): Promise<void> {
  35 |         await this.myAccountDropdown.click({ force: true });
  36 |     }
  37 | 
  38 |     /**
  39 |      * Clicks on the Register link from the My Account dropdown
  40 |      */
  41 |     async clickRegister(): Promise<void> {
  42 |         await this.openMyAccountDropdown();
  43 |         await this.registerLink.click();
  44 |     }
  45 | 
  46 |     /**
  47 |      * Clicks on the Login link from the My Account dropdown
  48 |      */
  49 |     async clickLogin(): Promise<void> {
  50 |         await this.openMyAccountDropdown();
  51 |         await this.loginLink.click();
  52 |     }
  53 | 
  54 |     /**
  55 |      * Searches for a product using the search field
  56 |      * @param productName - Product name to search for
  57 |      */
  58 |     async searchProduct(productName: string): Promise<void> {
  59 |         await this.searchInput.fill(productName);
> 60 |         await this.searchButton.click();
     |                                 ^ Error: locator.click: Test timeout of 30000ms exceeded.
  61 |     }
  62 | 
  63 |     /**
  64 |      * Returns the home page URL
  65 |      */
  66 |     getUrl(): string {
  67 |         return process.env.WEB_APP_URL || 'http://localhost/opencart/upload/';
  68 |     }
  69 | }
```