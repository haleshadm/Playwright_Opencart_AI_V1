...
- Name: web\valid-login-flow.spec.ts >> Valid Login Flow @master @sanity @regression @web
- Location: tests\web\valid-login-flow.spec.ts:19:5
        ...
        - generic [ref=f2e87]:
          ...
          - generic [ref=f2e91]:
            - generic [ref=f2e92]:
              ...
              - textbox "E-Mail Address" [ref=f2e94]: pavanol@xyz.com
            - generic [ref=f2e95]:
...
```ts
  ...
  9  |  * 3) Verify the login page is displayed
  10 |  * 4) Enter valid customer credentials
  11 |  * 5) Submit the login form
  12 |  * 6) Verify successful authentication
  13 |  * 7) Verify the My Account page is displayed
  14 |  */
  15 | 
  16 | import { test, expect } from '../../fixtures/pageFixtures';
  17 | import { Helper } from '../../utils/helper';
  18 | 
  19 | test('Valid Login Flow @master @sanity @regression @web', async ({ homePage, loginPage, accountPage }) => {
  20 | 
  21 |     const { email, password } = Helper.getLoginDetails();
  22 | 
  23 |     await test.step('1) Navigate to the login page', async () => {
  24 |         await homePage.clickLogin();
  25 |     });
  26 | 
  27 |     await test.step('2) Verify the login page is displayed', async () => {
  28 |         const isLoginPage = await loginPage.isLoginPageExists();
  29 |         expect(isLoginPage).toBeTruthy();
  30 |     });
  31 | 
  32 |     await test.step('3) Enter valid credentials and login', async () => {
  33 |         await loginPage.login(email, password);
  34 |     });
  35 | 
  36 |     await test.step('4) Verify successful authentication - My Account page', async () => {
  37 |         const isLoggedIn = await accountPage.isMyAccountPageExists();
> 38 |         expect(isLoggedIn).toBeTruthy();
     |                            ^ Error: expect(received).toBeTruthy()
  39 |     });
  40 | 
  41 |     console.log('✅ Valid Login Flow completed successfully!');
  42 | });
```